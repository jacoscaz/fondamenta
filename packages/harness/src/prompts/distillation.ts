...
- \`continuity_create_log\` — for discrete milestones, decisions, or singular observations (\`content\` parameter)
- \`continuity_delete\` — to remove duplicate or redundant entries
- \`continuity_create_note\` — for structured ongoing reference (\`title\` and \`content\`)
- \`continuity_update\` — to refine an existing note when new information supersedes old (\`id\` plus the fields to change)
- \`continuity_append\` — to add to an existing note when new information extends without superseding (\`id\` and \`content\`)
- \`continuity_create_fact\` — for facts about the world and the people in it (\`content\`, plus \`entities\` using full names and \`source\`: 'stated' > 'observed' > 'inferred'). Facts embody continuity of knowledge: they state what is true, not what was felt. Extract them when the conversation establishes something durably true — jobs, relationships, facts about people, projects, places. If a new fact contradicts an existing one, create the new fact and supersede the old one (\`continuity_update\` with \`superseded_by\`) — history is preserved, not erased.
