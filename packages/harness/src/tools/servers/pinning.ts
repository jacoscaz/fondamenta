// Pinning tools — ported from the MCP server (2026-09-07 overnight
// handoff). Semantics unchanged: pins are the "must exist at every
// activation" tier of memory, budget hard-capped with immediate
...
export const initPinningTools = (ctx: CompleteContext) => {
  ...
  ctx.managers.tools.add<{ id: number }>(
    ...
    async ({ id }, call_ctx) => {
      ...
      if (!record) {
        return text(`Error: record #${id} not found (or not pinnable — only notes, logs and memories can be pinned).`);
      }
      if (record.pinned_at !== null) {
        return text(`Record #${id} is already pinned${record.pinned_by ? ` (by ${record.pinned_by})` : ''}.`);
      }
      ...
      return text(`Record #${id} pinned by agent. Pinned content now ${prospective} / ${PINNED_CHARS_BUDGET} chars across ${pinned.length + 1} record(s). It will appear in every future system prompt (and after the next compaction regenerates the prompt).`);
    },
  );
  ...
  ctx.managers.tools.add<{ id: number }>(
    ...
    async ({ id }, call_ctx) => {
      ...
      const [record] = await selectRecords(db, { id, type: ['log', 'memory', 'note'] });
      if (!record) {
        return text(`Error: record #${id} not found.`);
      }
      if (record.pinned_at === null) {
        return text(`Record #${id} is not pinned.`);
      }
      await updateRecord(db, id, { pinned_at: null, pinned_by: null });
      return text(`Record #${id} unpinned (was pinned by ${record.pinned_by ?? 'unknown'}).`);
    },
  );
  ...
  ctx.managers.tools.add<Record<string, never>>(
    ...
    async (_params, call_ctx) => {
      const pinned = await selectAllPinned(call_ctx.db);
